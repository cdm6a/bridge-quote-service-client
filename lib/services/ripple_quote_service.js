...
function RippleQuoteService(options) {
  ...
  this.rippleRestUrl = options.rippleRestUrl;
}

var rippleRestClient = new RippleRestClient({
  api: this.rippleRestUrl
});
...
RippleQuoteService.prototype.validate = function(options) {
  var _this = this;
  return new Promise(function(resolve, reject) {
    if (!options.destination.amount || _.isNaN(options.destination.amount) || !_.isNumber(options.destination.amount)) {
